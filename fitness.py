...
def make_fitness(function, greater_is_better, wrap=False):
    ...
    if wrap:
        return _Fitness(function=jl.wrap_non_picklable_objects(function),
...
def _chi2_square(y, y_pred, w, e=0.15):
    """Calculate the chi2 square."""
    #e = y / y * (abs(max(y * e)) + abs(min(y * e))) / 2.0
    #e = y / y * (abs(max(y * e)) + abs(min(y * e))) / 2.0
    #e = y / y * (max(abs(y) * e) + min(abs(y) * e)) / 2.0
    e = y / y * max(abs(y) * e)
    #e = np.ones(len(y)) 
    #e = y / y * sum(abs(y) * e) / len(y)
    #e = y * e + 0.001
    return np.average((y - y_pred)**2 / e**2, weights=w)
...
log_loss = _Fitness(function=_log_loss,
                    greater_is_better=False)
chi2_square = _Fitness(function=_chi2_square,
                    greater_is_better=False)
