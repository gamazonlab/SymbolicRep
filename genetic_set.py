...
from ._program_set import _Program
...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    for i in range(n_programs):
        ...
        if parents is None:
            ...
            if method < method_probs[0]:
                ...
                if False:#label == 198:
                    print("crossover", label)
                    parent.print_program(pflag=False)
                    donor.print_program(pflag=False)
                    pflag = True
                    program, p0, removed, remains = parent.crossover(donor.program, donor.p0,
                                                             random_state, True)
                ...
                if False:#label == 254:
                    ...
                    print()
                    pflag = True
                    program, p0, removed, _ = parent.subtree_mutation(random_state, prior)
                else:
                    program, p0, removed, _ = parent.subtree_mutation(random_state, prior)
                #program, removed, _ = parent.subtree_mutation(random_state, prior)
                ...
                # hoist_mutation
                #parent.print_program(pflag=False)
                program, p0, removed = parent.hoist_mutation(random_state)
                ...
                #pflag = True
                # point_mutation
                #parent.print_program()
                if False:#label == 3189:
                    parent.print_program(pflag=False)
                    program, p0, mutated = parent.point_mutation(random_state, pflag=True)          
                else:
                    program, p0, mutated = parent.point_mutation(random_state) 
                #program, mutated = parent.point_mutation(random_state)
                genome = {'method': 'Point Mutation',
                          'parent_idx': parent_index,
                          'parent_nodes': mutated}
            else:
                #print("reproduction", label)
                label += 1
                #pflag = True
                # reproduction
                program, p0 = parent.reproduce()
        ...
        #            if i == last + 1:
        #                print()
        #                print("false program", label, method)
        #                print(method_probs)
        #        i += 1
        ...
        #if parents is None:
        #    program.print_program(pflag=False)
        ...
        if False:    
        #if program.raw_fitness_ < 1e-5:
            print(i, program.raw_fitness_)
    ...
    #programs[position].print_program()
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def cal_p_i(self, population):
        p_i = {}
        for program in population:
            for p in program.p_ij:
                if p in p_i:
                    p_i[p] += program.p_ij[p]
                else:
                    p_i[p] = program.p_ij[p]
        
        for p in p_i:
            p_i[p] /= len(population)
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for gen in range(prior_generations, self.generations):
            ...
            #e = 0.15 - 0.05 * gen / self.generations
            #print(gen, e)
            ...
            if self.parsimony_coefficient == 'auto':
                parsimony_coefficient = (np.cov(length, fitness)[1, 0] /
                                         np.var(length))
            for program in population:
                program.fitness_ = program.fitness(parsimony_coefficient, p_i)
            ...
            self._best_programs.append(best_program)
            ...
            if self.verbose:
                ...
                #best_program.print_program(pflag=False)
...
class SymbolicRegressor(BaseSymbolic, RegressorMixin):
    ...
    def predict(self, X):
        ...
        #y = self._program.execute_flat(X)
